...
回放：
    python A_replay_trajectory.py replay waypoints_YYYYMMDD_HHMMSS.json
...
def smooth_move(robot: Panthera, start: np.ndarray, end: np.ndarray, duration: float) -> None:
...
def replay_waypoints(args: argparse.Namespace) -> None:
    trajectory_path = Path(args.file).expanduser().resolve()
    _, waypoints = load_waypoints(trajectory_path)
    config_path = Path(args.config).expanduser().resolve()
    robot = make_robot(config_path)
    current_position, _ = warm_up_state(robot)

    try:
        print(f"\n载入 {len(waypoints)} 个预设点：{trajectory_path.name}")
        ...
        for waypoint_number, waypoint in enumerate(waypoints, start=1):
            ...
            print(
                f"\n点 {waypoint_number}/{len(waypoints)}："
...
def build_parser() -> argparse.ArgumentParser:
    ...
    replay = subparsers.add_parser("replay", help="平滑回放 JSON 预设点")
    replay.add_argument("file", help="record 命令生成的 waypoint JSON 文件")
